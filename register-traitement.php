            ...
            if (strlen($prenom) <= 100){
                if(strlen($email) <= 100){
                  if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                    ...
                    }else{ header('Location: register.php?reg_err=password_lenght');}
                }else{ header('Location: register.php?reg_err=valid');}
...
}else{ header('Location: register.php?reg_err=emptyfield');}